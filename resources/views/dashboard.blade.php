...
<div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
    ...
    <div class="overflow-hidden rounded-lg bg-white px-4 py-5 shadow sm:p-6">
        ...
        <dd class="mt-1 text-3xl font-semibold tracking-tight text-gray-900">{{ $stats['total_posts'] ?? 0 }}</dd>
    </div>
    <div class="overflow-hidden rounded-lg bg-white px-4 py-5 shadow sm:p-6">
        <dt class="truncate text-sm font-medium text-gray-500">Active Subscribers</dt>
        <dd class="mt-1 text-3xl font-semibold tracking-tight text-gray-900">{{ $stats['total_subscribers'] ?? 0 }}</dd>
    </div>
    <div class="overflow-hidden rounded-lg bg-white px-4 py-5 shadow sm:p-6">
        <dt class="truncate text-sm font-medium text-gray-500">Active Employees</dt>
        <dd class="mt-1 text-3xl font-semibold tracking-tight text-gray-900">{{ $stats['total_employees'] ?? 0 }}</dd>
    </div>
</div>
...
<div class="mt-8 grid grid-cols-1 gap-5 lg:grid-cols-3">
    ...
    <div class="rounded-lg bg-white shadow p-6 lg:col-span-2">
        <h3 class="text-lg font-medium text-gray-900 mb-4">Welcome, {{ $user_name ?? 'User' }}!</h3>
        <p class="text-gray-600">Your multi-module platform is running with the Blade template engine. Use the navigation above to access CMS, Email Marketing, and HRM modules.</p>
    </div>
</div>
@endsection
